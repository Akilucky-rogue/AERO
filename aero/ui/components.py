...
def render_kpi_card(
    ...
    value_or_subtitle: str = None,
    color: str = _PURPLE,
    subtitle: str = None,
    icon: str = None,
):
    """Render a branded KPI tile.  Supports two calling conventions:

    New (preferred) — call inside an st.column context:
        render_kpi_card(label, value, subtitle=None, color=_PURPLE)

    Legacy — pass the column explicitly (still works):
        render_kpi_card(col, label, value, color=_PURPLE, subtitle=None)
    """
    # Detect which calling convention is in use
    if hasattr(label_or_col, "markdown"):
        # Legacy: first arg is a Streamlit column object
        col      = label_or_col
        label    = label_or_value
        value    = value_or_subtitle
        # subtitle stays as kwarg
        _md = col.markdown
    else:
        # New: first arg is the label string
        label    = label_or_col
        value    = label_or_value
        if subtitle is None:
            subtitle = value_or_subtitle
        _md = st.markdown
