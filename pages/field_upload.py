...
def _process_famis(file_bytes: bytes, filename: str, file_type: str) -> pd.DataFrame:
    ...
    # Always upsert to Excel store as backup
    try:
...
if _db_live:
    ...
    render_info_banner(
        "PostgreSQL Active",
        f"Database is connected. <b>{_row_cnt:,}</b> FAMIS records currently stored. "
        "Uploads are automatically persisted to PostgreSQL and backed up locally to <code>docs/famis/</code>.",
        accent=_GREEN,
    )
else:
    render_info_banner(
        "Excel Mode",
        "PostgreSQL is not configured — uploads are saved to <code>data/FAMIS_UPLOADED_FILES.xlsx</code> "
        "and archived locally to <code>docs/famis/</code>. "
        "Set up PostgreSQL to enable full historical analytics.",
        accent=_ORANGE,
    )
...
with kpi_c4:
    render_kpi_card("Master Data", "Loaded" if master_in_session is not None else "Not Loaded",
                    "station_planner_master.xlsx", color=_GREEN if master_in_session is not None else _ORANGE)
...
<div style="background:linear-gradient(90deg,#4D148C 0%,#7B2FBE 100%);
    ...
    <div style="color:#E8D5FF;font-size:12px;margin-top:4px;">
        Weekly FAMIS REPORT (.xlsx) or Daily/Monthly volume exports (.xlsx / .csv / .txt)
    </div>
</div>
...
with f_col1:
    famis_file = st.file_uploader(
        "Upload FAMIS file (.xlsx, .xls, .csv, .txt)",
    ...
    with st.spinner(f"Processing {famis_file.name} …"):
        try:
            ...
            st.success(
                ...
                f"{'Saved to PostgreSQL' if db_ok else 'Saved to Excel store'}"
            )
        except Exception as exc:
...
master_file = st.file_uploader(
    "Upload Facility Master file (.xlsx / .csv)",
    type=["xlsx", "xls", "csv"],
    key="master_uploader",
    label_visibility="collapsed",
)
...
if master_file is not None:
    try:
        ...
        if master_file.name.endswith(".csv"):
            mdf = pd.read_csv(io.BytesIO(raw))
        else:
            mdf = pd.read_excel(io.BytesIO(raw))
        ...
        if "loc_id" not in mdf.columns:
            ...
            st.success(
                f"✅ **{master_file.name}** loaded — "
                f"**{len(mdf)}** stations · "
                f"Columns: {', '.join(c for c in mdf.columns if c in _MASTER_VALID)}"
            )
    except Exception as exc:
...
<div style="background:linear-gradient(90deg,#145A32 0%,#1E8449 100%);
    ...
    <div style="color:#D5F5E3;font-size:12px;margin-top:4px;">
        Last 10 FAMIS uploads stored in this session / database
    </div>
</div>
""", unsafe_allow_html=True)
...
if _db_live:
    try:
        ...
        st.info("Upload history unavailable (DB not connected).")
else:
    # Show from Excel store
    try:
        hist_df = read_famis_uploads()
        if not hist_df.empty and "date" in hist_df.columns:
            summary = (
                hist_df.groupby("uploaded_at" if "uploaded_at" in hist_df.columns else hist_df.columns[0])
...
with st.expander("⬇️  Download Data Templates", expanded=False):
    ...
    with dl1:
        ...
        tpl = pd.DataFrame([["2025-01-01","ABCD",1000,600,400,50,30,200,10]],
        ...
        st.download_button("⬇️ FAMIS Template", buf.getvalue(), "FAMIS_Template.xlsx",
    ...
    with dl2:
        ...
        tpl2 = pd.DataFrame([["ABCD",50000,35000,25,12]],
            columns=["loc_id","total_facility_area","ops_area","current_total_agents","current_total_couriers"])
        buf2 = io.BytesIO()
        with pd.ExcelWriter(buf2, engine="openpyxl") as w:
            tpl2.to_excel(w, index=False, sheet_name="Master")
        st.download_button("⬇️ Master Template", buf2.getvalue(), "Master_Template.xlsx",
                           "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                           use_container_width=True)
