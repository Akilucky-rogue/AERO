    ...
    if famis_in_session_now is not None and not famis_in_session_now.empty:
        try:
            ...
            with st.spinner("⚙️ Generating AERO Analytics Report (HTML) …"):
                ...
                _dname  = st.session_state.get("famis_file_name", famis_file.name if famis_file else "AERO_Report")
            ...
            import logging
            logging.getLogger(__name__).warning("HTML report generation failed: %s", _html_exc)
